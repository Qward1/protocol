...
- Контроль исполнения: `PATCH /api/tasks/{id}`, `POST .../execution` (что сделано),
  ...
  `POST .../send-max` (отправить карточку поручения в группу MAX).
...
- Локальное хранение поручений в БД приложения; опциональное дублирование во внешний сервис (`execution_control.enabled`).
